...
<div class="app-container">
  <nav class="sidebar">
    <div class="logo"><span class="sicon">🌍</span><h2>إدارة الصفوف</h2><p>دراسات اجتماعية</p></div>
    <nav>
...
function shareText(text) { if (navigator.share) navigator.share({ title: 'إدارة الصفوف', text }); else window.open('https://wa.me/?text=' + encodeURIComponent(text)); }
...
function loadAireport() {
  ...
  const highMonth1 = allStudents.filter(s => s.month1 >= 12);
  const highMonth2 = allStudents.filter(s => s.month2 >= 12);
  ...
  let h = `<div class="page"><h2>🤖 تحليل الذكاء الاصطناعي - دراسات اجتماعية
    <div class="page-actions">
